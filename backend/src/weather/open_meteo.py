...
async def _fetch_from_api(lat: float, lng: float, location_meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Antecedent Metrics Calculation
    rain_list = hourly.get("rain", []) or hourly.get("precipitation", [])
    pressure_list = hourly.get("surface_pressure", [])
    
    # 72h antecedent rainfall
    antecedent_rainfall_72h = sum(filter(None, rain_list[-72:])) if len(rain_list) >= 72 else sum(filter(None, rain_list))
    
    # 24h pressure tendency
    pressure_tendency_24h = 0.0
    if len(pressure_list) >= 25:
        p_now = pressure_list[-1]
        p_24h_ago = pressure_list[-25]
        if p_now is not None and p_24h_ago is not None:
            pressure_tendency_24h = p_now - p_24h_ago

    # CAPE persistence (consecutive hours > 1000 J/kg)
    cape_persistence_hours = 0
    for c in reversed(cape_list[-72:]):
        if c is not None and c > 1000:
            cape_persistence_hours += 1
        else:
            break
    ...
    normalized_weather = {
        ...
        "iwv_kg_m2": iwv_val,
        "antecedent_rainfall_72h": round(antecedent_rainfall_72h, 2),
        "pressure_tendency_24h": round(pressure_tendency_24h, 2),
        "cape_persistence_hours": cape_persistence_hours,
