...
class IMDNowcastService:
    def __init__(self):
        self.api_key = os.getenv("IMD_API_KEY", "")
        self.base_url = "https://api.imd.gov.in/public/api"

    async def get_nowcast(self, district: str) -> List[UnifiedAlert]:
        url = f"{self.base_url}/nowcast/district"
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for item in data.get("hazards", []):
                    alerts.append(UnifiedAlert(
                        alert_id=f"IMD-NC-{district}-{now}",
                        ...
                        source="IMD",
                        source_type="official_nowcast",
                        official_warning=item.get("hazard", "UNKNOWN")
                    ))
                return alerts
        except Exception as e:
            logging.error(f"IMDNowcastService Error: {e}")
            return []
