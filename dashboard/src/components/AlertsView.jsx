...
export default function AlertsView({ showToast, onNavigateTab }) {
  ...
  const dispatchAIAlert = async () => {
    ...
    setAiResult(null);
    try {
      const response = await fetch('http://localhost:8000/api/alerts/ai-dispatch', {
        method: 'POST',
      ...
      showToast(`AI Warning generated in Regional Languages!`);
    } catch (e) {
      ...
      <div className="tac-clean-top-row">
        ...
        <div className="tac-clean-threat-card" style={{ 
          ...
          <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {aiResult && (
              <div className="ai-dispatch-result" style={{ margin: '8px 0', border: '1px solid rgba(147, 51, 234, 0.4)' }}>
                <div className="ai-dispatch-source">Generated by {aiResult.source}</div>
                <div className="ai-alert-box" style={{ background: 'rgba(0,0,0,0.4)', borderRadius: '4px' }}>
                  <strong>EN:</strong> {aiResult.english}
                </div>
                <div className="ai-alert-box regional-text" style={{ background: 'rgba(0,0,0,0.4)', borderRadius: '4px' }}>
                  <strong>Regional:</strong> {aiResult.regional}
                </div>
              </div>
            )}
