        ...
        <div className="aira-threat-card" style={{ backgroundColor: 'rgba(15, 23, 42, 0.4)', border: '1px dashed #334155' }}>
          ...
          <div className="aira-threat-text-col">
            ...
            <div className="aira-location">
              <MapPinIcon />
              <span>{predictions.location?.name || 'Unknown'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="aira-weather-box">
          ...
          <div className="aira-weather-grid">
            <div className="aira-w-row">
              ...
              <span className="aira-w-val">{predictions.weather?.temperature_c ?? '--'} °C</span>
            </div>
            <div className="aira-w-row">
              <span className="aira-w-label">Pressure</span>
              <span className="aira-w-val">{predictions.weather?.pressure_hpa ?? '--'} hPa</span>
            </div>
            
            <div className="aira-w-row">
              <span className="aira-w-label">Rain</span>
              <span className="aira-w-val aira-w-val-blue">{predictions.weather?.rain_mm_h ?? '--'} mm/h</span>
            </div>
            <div className="aira-w-row">
              <span className="aira-w-label">CAPE</span>
              <span className="aira-w-val aira-w-val-orange">{predictions.weather?.cape_jkg ?? '--'} J/kg</span>
            </div>
            
            <div className="aira-w-row">
              <span className="aira-w-label">CIN</span>
              <span className="aira-w-val">{predictions.weather?.cin_jkg ?? '--'} J/kg</span>
            </div>
            <div className="aira-w-row">
              <span className="aira-w-label">IWV</span>
              <span className="aira-w-val aira-w-val-blue">{predictions.weather?.iwv_kg_m2 ?? '--'} kg/m²</span>
            </div>
          </div>
    ...
    <div className="aira-container">
      ...
      <div className="aira-threat-card">
        ...
        <div className="aira-threat-text-col">
          ...
          <h3 className="aira-threat-subtitle" style={{ color: subtitleColor }}>Risk Score: {Number(score || 0).toFixed(2)}</h3>
          <div className="aira-location">
            <MapPinIcon />
            <span>{loc}</span>
          </div>
      ...
      <div className="aira-narrative">
        Current conditions in {loc} indicate a CAPE of <strong>{ap.cape_jkg} J/kg</strong> and IWV of 
        <strong> {ap.iwv_kg_m2} kg/m²</strong>. The multi-task deep learning model predicts a 
        <strong> {Number(p.flash_flood_probability * 100 || 0).toFixed(1)}% </strong> probability of Flash Flood and 
        <strong> {Number(p.cloudburst_probability * 100 || 0).toFixed(1)}% </strong> probability of Cloudburst.
      </div>
      ...
      <div className="aira-metrics-grid">
        ...
        <div className="aira-metric-box">
          <div className="aira-metric-val">
            <ZapIcon />
            <span>{ap.cape_jkg ?? '--'} <span>J/kg</span></span>
          </div>
        ...
        <div className="aira-metric-box">
          <div className="aira-metric-val">
            <ClockIcon />
            <span>{Number(p.thunderstorm_probability * 100 || 0).toFixed(1)} <span>%</span></span>
          </div>
      ...
      <div className="aira-action-box">
        ...
        <div className="aira-action-text" style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span>Flash Flood: {Number(p.flash_flood_probability * 100 || 0).toFixed(1)}%</span>
          <span>Cloudburst: {Number(p.cloudburst_probability * 100 || 0).toFixed(1)}%</span>
        </div>
      </div>
      ...
      <div className="aira-weather-box">
        ...
        <div className="aira-weather-grid">
          <div className="aira-w-row">
            <span className="aira-w-label">Temperature</span>
            <span className="aira-w-val">{ap.temperature_c ?? '--'} °C</span>
          </div>
          <div className="aira-w-row">
            <span className="aira-w-label">Pressure</span>
          ...
          <div className="aira-w-row">
            <span className="aira-w-label">CAPE</span>
            <span className="aira-w-val aira-w-val-orange">{ap.cape_jkg ?? '--'} J/kg</span>
          </div>
          
          <div className="aira-w-row">
            <span className="aira-w-label">CIN</span>
            <span className="aira-w-val">{ap.cin_jkg ?? '--'} J/kg</span>
          </div>
          <div className="aira-w-row">
            <span className="aira-w-label">IWV</span>
