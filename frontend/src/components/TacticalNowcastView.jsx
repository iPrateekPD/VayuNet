...
export default function TacticalNowcastView({ onDispatchAlert, showToast, onNavigateTab, globalSelectedLocation, setGlobalSelectedLocation }) {
      ...
      try {
        ...
        if (data) {
          ...
          if (data.risk && data.risk.level) {
            setAiSeverity(data.risk.level.toLowerCase());
          } else {
            setAiSeverity('gray');
          }
        }
  ...
  const handleSelectBookmark = (loc) => {
    ...
    if (sectorCoords[loc]) {
      setCurrentCenter([...sectorCoords[loc]]);
      setCurrentZoom(9.5);
        ...
        <div className="tac-main-dashboard">
          ...
          <div className="tac-clean-grid">
            ...
            >
              ...
              {sectorCoords[selectedSector] && (
                <Marker
                  position={sectorCoords[selectedSector]}
                  icon={createHtmlIcon(`
